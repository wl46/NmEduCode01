...
public class MediaUploadService {
    ...
    private String getChunkFileFolderPath(String fileMd5){
     return uploadPath+fileMd5.substring(0,1)+"/"+fileMd5.substring(1,2)+"/"+fileMd5+"/"+fileMd5+"/chunks/";
    }
    ...
    //上传分块
    public ResponseResult ulpoadChunk(MultipartFile file, String fileMd5, Integer chunk) throws IOException {
        //检查分块目录，如果不存在则要自动创建
        //得到分块目录
        String chunkFileFolderPath = this.getChunkFileFolderPath(fileMd5);
        //得到分块文件的路径
        String chunkFPath = chunkFileFolderPath + chunk;
        ...
        try {
            inputStrean=file.getInputStream();
            outputStream = new FileOutputStream(new File(chunkFPath));
            IOUtils.copy(inputStrean,outputStream);
        } catch (Exception e) {
            e.printStackTrace();
        }finally {
            try {
    ...
    public ResponseResult mergechunks(String fileMd5, String fileName, Long fileSize, String mimetype, String fileExt) {
        ...
        String mediaId = mediaFile.getFilePath();
        //向MQ发送视频处理消息
